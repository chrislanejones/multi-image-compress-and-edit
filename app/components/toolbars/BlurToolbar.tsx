...
const BlurSecondaryToolbar = () => {
  const { blurAmount, blurRadius, setBlurAmount, setBlurRadius } = useEditorStore();

  return (
    <div className="flex items-center gap-4 p-2 bg-gray-700 rounded-lg mb-4 w-full">
      <div className="flex-1">
        <label htmlFor="blur-amount" className="text-sm font-medium block mb-1 text-white">Blur Amount: {blurAmount}px</label>
        <Slider id="blur-amount" min={1} max={20} step={1} value={[blurAmount]} onValueChange={(v) => setBlurAmount(v[0])} />
      </div>
      <div className="flex-1">
        <label htmlFor="blur-radius" className="text-sm font-medium block mb-1 text-white">Brush Size: {blurRadius}px</label>
        <Slider id="blur-radius" min={5} max={50} step={1} value={[blurRadius]} onValueChange={(v) => setBlurRadius(v[0])} />
      </div>
    </div>
  );
};
...
export const BlurToolbar = () => {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <Button onClick={onApplyBlur} variant="default" className="h-9"><Check className="mr-2 h-4 w-4" /> Apply Blur</Button>
