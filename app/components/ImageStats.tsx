...
export default function ImageStats({ selectedImage }: ImageStatsProps) {
  ...
  const { originalSize, compressedSize, compressionRatio } = selectedImage.metadata;

  return (
    <Card className="rounded-lg border shadow-sm bg-gray-800 text-white border-gray-700">
      <CardHeader className="p-3 pb-0">
        <CardTitle className="text-lg font-semibold">Compression Stats</CardTitle>
      </CardHeader>
      <CardContent className="p-4">
        <div className="grid grid-cols-3 gap-4 text-sm">
          ...
          <div>
            ...
            <p className="text-green-400">{formatBytes(compressedSize || originalSize || 0)}</p>
          </div>
          <div>
            <span className="font-medium text-muted-foreground">Savings:</span>
            <p className="text-green-400 font-semibold">{compressionRatio || 0}%</p>
          </div>
        </div>
      </CardContent>
    </Card>
